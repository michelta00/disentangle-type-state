...
Simulation data obtained via snakemake workflow from https://github.com/HelenaLC/type-state.
...
ggplot(
  ...
  scale_colour_manual(values = cbf_palette[3:5]) +
  labs(title = "",
       x = "PC1",
       y = "PC2",
       colour = "Group (Cell Type)") +
  theme_classic() +
  theme(
...
ggplot(
  data = cd,
  mapping = aes(x = PC1, y = PC2, colour = group_id)
) +
  geom_point(size = 1.4, alpha = 0.7) +
...
ggplot(
  data = cd,
  mapping = aes(x = PC1, y = PC2, colour = sample_id)
) +
  geom_point(size = 1.4, alpha = 0.7) +
  scale_colour_manual(values = cbf_palette[3:8]) +
  labs(title = "",
       x = "PC1",
       y = "PC2",
       colour = "Sample") +
